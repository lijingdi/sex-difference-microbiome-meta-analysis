...
V <- vcalc(vi, cluster=Authors, obs=obs, data = alpha_es_treat[alpha_es_treat$stats == "lnRR" & alpha_es_treat$Sex == "female",], rho=0.5) #model independence among multiple effect sizes from the same study -- they share the same control group
...
fit_he <- rma.mv(yi, V, 
                  ...
                  struct = "DIAG",
                  mod = ~metric -1,
                  data = alpha_es_treat[alpha_es_treat$stats == "lnRR" & alpha_es_treat$Sex == "female",], 
                  test = "t", 
                  method = "REML",
                  sparse = TRUE) #add to make it faster
...
```{r beta dispersion ES calculation for treatment vs. control within sex}
###beta-dispersion
head(tb_all)
tb_all_long <- tb_all %>%
    mutate(group_general = case_when(
...
```{r Beta dispersion meta-meta model and ES visualized using orchaRd treatment vs. control}
###heteroscedastic model
beta_es_treat <- beta_es_treat %>% filter(!if_all(everything(), is.na))
beta_es_treat$obs <- 1:nrow(beta_es_treat)

V <- vcalc(vi, cluster=Authors, obs=obs, data = beta_es_treat[beta_es_treat$Sex == "male",], rho=0.5) #model independence among multiple effect sizes from the same study -- they share the same control group
...
fit_he <- rma.mv(yi, V, 
                  ...
                  struct = "DIAG",
                  mod = ~metric -1,
                  data = beta_es_treat[beta_es_treat$Sex == "male",], 
                  test = "t", 
                  method = "REML",
                  sparse = TRUE) #add to make it faster

orchaRd::mod_results(fit_he, mod="metric", group = "Authors")
p1 <- orchaRd::orchard_plot(fit_he, mod = "metric", group = "Authors", xlab = "lnRR")
...
summary(fit_ho)
orchaRd::mod_results(fit_ho, mod="metric", group = "Authors")
p1 <- orchaRd::orchard_plot(fit_ho, mod = "metric", group = "Authors", xlab = "lnRR")
