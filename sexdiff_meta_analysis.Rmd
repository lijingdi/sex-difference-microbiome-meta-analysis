...
```{r Alpha-div summary ES meta-meta not recommended as each alpha-div metrics gave specific info that is useful}
```


```{r Alpha-div ES visualization}
######visualize forest plot, all ESs######
color_pal <- c("#0072B2", "#D55E00", "#56B4E9", "#009E73", "#F0E442", "#CC79A7")
alpha_es <- alpha_es %>%
...
p1<- alpha_es %>%
  ...
  geom_point(shape = 16, aes(color=metric)) + 
...
calculate_smd <- function(dat, mod, metric=c("unifrac_dist", "wunifrac_dist", "bray_dist", "jac_dist")) {
  beta_smd <- data.frame(CN=numeric(0), EN=numeric(0),
                       yi=numeric(0), vi=numeric(0), Authors=character(0), metric=character(0))
  for (i in unique(dat$metric)) {
      dat_sub <- dat[dat$metric == i,]
      add_smd <- escalc(measure = "SMD", m1i=mean_male, m2i=mean_female, sd1i=sd_male, sd2i=sd_female, n1i=n_male, n2i=n_female, data=dat_sub, vtype = "UB")
      beta_smd <- rbind(beta_smd, add_smd)
  }
  return(beta_smd)
}


tb_all_long %>% calculate_smd()
