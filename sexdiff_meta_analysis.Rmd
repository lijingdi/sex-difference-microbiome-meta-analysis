...
sub <- subset_samples(dat33, Authors == study)
sample_data_df <- data.frame(sample_data(sub))
sample_data_df %>% group_by(genotype, host_sex) %>% summarise(n = n())
```
...
#Lyte et al. 2022: host_diet; Zou et al. 2021: experimental_treatment_raw; Song et al. 2021: experimental_treatment_raw; Velasco et al. 2021:strain; Li et al. 2020: experimental_treatment_raw; Son et al. 2019: experimental_treatment_raw; Cheema & Pluznick 2019: experimental_treatment; Zajac et al. 2022: genotype; Sun et al. 2020: experimental_treatment_raw; Kozik et al. 2019: experimental_treatment_raw; Tam et al. 2020: experimental_treatment_raw; Ramakrishna et al. 2020: experimental_treatment; Song et al. 2020: experimental_treatment_raw; Ibrahim et al. 2019: experimental_treatment_raw; Weng et al. 2019: experimental_treatment_raw； Wolfe et al. 2020：experimental_treatment_raw; Tashiro et al. 2020:experimental_treatment_raw; Song et al. 2022: experimental_treatment; Gubert et al. 2022: experimental_treatment_raw

group <- "experimental_treatment"
for (subgroup in unique(sample_data_df[[group]])) {
  ...
  if (!subgroup %in% unique(dat_alpha$subgroup)) {
...
head(alpha_es) #132 ES for each metric
alpha_es[is.infinite(alpha_es)]
...
write.csv(alpha_es, "./results/SRS_data/alpha_es.csv")
...
orchaRd::orchard_plot(fit_he, mod = "1", group = "Authors", xlab = "lnCVR")
orchaRd::orchard_plot(fit_he, mod = "metric", group = "Authors", xlab = "lnCVR")

###homoscedastic model
fit_ho <- rma.mv(yi, vi, slab = Authors,random = list(~ 1 | Protocol/Authors), mod = ~metric -1,
              data = alpha_es[alpha_es$stats == "lnCVR",], method = "REML")
...
orchaRd::orchard_plot(fit_ho, mod = "1", group = "Authors", xlab = "lnCVR")
...
orchaRd::mod_results(fit_he, mod="1", group = "Authors")
orchaRd::mod_results(fit_he, mod="metric", group = "Authors")
orchaRd::orchard_plot(fit_he, mod = "1", group = "Authors", xlab = "lnVR")
...
orchaRd::orchard_plot(fit_ho, mod = "1", group = "Authors", xlab = "lnVR")
...
###heteroscedastic model
fit_he <- rma.mv(yi, vi, slab = Authors,random = list(~1|metric, ~ 1 | Protocol/Authors), mod = ~metric -1,
              data = alpha_es[alpha_es$stats == "lnRR",], method = "REML") ##raise error, cannot achieve convergence 
plot(fitted(fit_he), residuals(fit_he))
orchaRd::mod_results(fit_he, mod="1", group = "Authors")
orchaRd::mod_results(fit_he, mod="metric", group = "Authors")
orchaRd::orchard_plot(fit_he, mod = "1", group = "Authors", xlab = "lnRR")
...
orchaRd::orchard_plot(fit_ho, mod = "1", group = "Authors", xlab = "lnRR")
...
betaCombined %>% 
  filter(pval < 0.05) 
write.csv(betaCombined, "./results/SRS_data/betaCombined.csv")
...
orchaRd::mod_results(fit_he, mod="1", group = "Authors")
orchaRd::mod_results(fit_he, mod="metric", group = "Authors")
orchaRd::orchard_plot(fit_he, mod = "1", group = "Authors", xlab = "SMD")
...
orchaRd::orchard_plot(fit_ho, mod = "1", group = "Authors", xlab = "SMD")
...
betaCombined$metric <- factor(betaCombined$metric, levels=c("Richness", "Chao1", "Evenness", "Shannon", "Simpson", "Faith's PD"))
...
```{r Alpha-div ES, try to stratefy treatment groups}
head(alpha_es)
