...
##################################################################################################
#original functions from Senior et al. 2020 Research Synthesis Methods
lnRR2 <- function(CMean, CSD, CN, EMean, ESD, EN){
  yi<-log( EMean / CMean) + 
    0.5 * (((ESD^2) / (EN * EMean^2)) - ((CSD^2) / (CN * CMean^2)))	
  vi <- CSD^2 / (CN * CMean^2) + CSD^4 / (2 * CN^2 * CMean^4)  + 
    ESD^2 / (EN * EMean^2) + ESD^4 / (2 * EN^2 * EMean^4) 
...
# lnCVR2 (better estimator)
lnCVR2 <- function(CMean, CSD, CN, EMean, ESD, EN){
  yi<-log((ESD / EMean) / (CSD / CMean)) + (1 / (2 * (EN - 1))) - (1 / (2 * (CN - 1))) 
  yi<-yi + 0.5 * (((CSD^2) / (CN * CMean^2)) - ((ESD^2) / (EN * EMean^2)))	
  vi<-CSD^2 / (CN * CMean^2) + CSD^4 / (2 * CN^2 * CMean^4) +
    1 / (2 * (CN - 1)) + 1 / (2 * (CN - 1)^2) + 
...
#lnVR2 (better estimator)
lnVR2 <- function(CMean, CSD, CN, EMean, ESD, EN){
  yi<-log(ESD / CSD) + (1 / (2 * (EN - 1))) - (1 / (2 * (CN - 1))) 
  vi<- 1 / (2 * (CN - 1)) + 1 / (2 * (CN - 1)^2) + 
    1 / (2 * (EN - 1)) + 1 / (2 * (EN - 1)^2)
  print(data.frame(yi,vi))
...
#lnSD and lnMean, this is for checking the correlation between lnM and lnsd

lnSD_lnM <- function(Mean, SD, N){
  lnsd<-log(SD) + (1 / (2 * (N - 1))) 
  vlnsd<- 1 / (2 * (N - 1)) + 1 / (2 * (N - 1)^2)
  lnm <- log(Mean) + 0.5*(SD^2 / (N * Mean^2)) 
  print(data.frame(lnsd,vlnsd, lnm))
}
